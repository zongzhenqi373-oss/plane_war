...
Item 类是游戏中 所有道具的抽象基类，用于统一管理道具的基本属性和行为。在游戏中，道具是玩家击落敌机后或随机掉落的物体，玩家拾取后会获得特殊效果，例如护盾或子弹增强等。该类采用了 多态 + 工厂模式 设计，保证扩展性强，能够方便地添加新的道具类型。
	2，主要功能：
（1）定义道具的基本属性。包括IMAGE m_img：道具的图像资源；int m_x、int m_y：道具在游戏画布上的坐标；；random_device rd：用于生成随机位置，使道具在屏幕顶部随机掉落。
...
三，模块程序更新
	1，游戏模块更新：
（1）新增三个定时器：//定时自动关闭关卡提示弹窗 ::SetTimer(m_hwnd, 8, 1500, nullptr);     //每次进入一个关卡后，都会有弹窗提醒，但是为了不影响玩家游戏，必须设置定时器来定时关闭这个弹窗
				     //定时检测道具 ::SetTimer(m_hwnd, 9, 10, nullptr);                               //定时对玩家是否拾取道具进行检测，如果拾取相关道具，就要产生相应效果
				     //定时创建道具 ::SetTimer(m_hwnd, 10, 3000, nullptr);                         //定时创建道具，随机创建
